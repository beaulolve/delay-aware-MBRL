...
public class RosImagePublisher : MonoBehaviour
{
    ...
    float timeElapsed = 0;
    public Camera cam;
    public string topicName = "/image0";
    ...
    private void Update()
    {
        ...
        // Image Format
        Texture2D imageTexture = ReturnRGBImage(cam,cameraWidth,cameraHeight);  //imageͼƬ
        // Use GetRawTextureData since EncodeToPNG or JPG does not import the whole size of the data
        RosImage imageRos = CopyData(cam, imageTexture.GetRawTextureData(),"rgb8", true);   // ת��ΪrosͼƬ
        ros.Publish(topicName, imageRos);
    }
